...
#tensorflow_examples/courses/udacity_deep_learning/5_word2vec.ipynb
...
with graph.as_default(), tf.device('/cpu:0'):
  ...
  embed = embedding_ops.embedding_lookup(embeddings, train_dataset)
  ...
  optimizer = adagrad.AdagradOptimizer(1.0).minimize(loss)
  # Compute the similarity between minibatch examples and all embeddings.
  # We use the cosine distance:
  norm = tf.sqrt(tf.reduce_sum(tf.square(embeddings), 1, keepdims=True))
  normalized_embeddings = embeddings / norm
  valid_embeddings = embedding_ops.embedding_lookup(
